...
/**
 * Return a human-readable address type name from raw address bytes.
 */
export function getAddressTypeName(addressBytes) {
  const buf = Buffer.isBuffer(addressBytes)
    ? addressBytes
    : Buffer.from(addressBytes);
  ...
  if (type <= 7) return "Enterprise";
  if (type >= 14) return "Stake";
  return "Unknown";
}

/**
 * Re-encode raw address bytes for a specific network ID.
 * Swaps the lower nibble of the header byte and picks the correct HRP.
 */
export function bech32EncodeAddressForNetwork(addressBytes, networkId) {
  const buf = Buffer.isBuffer(addressBytes)
    ? addressBytes
    : Buffer.from(addressBytes);
  const addressType = (buf[0] >> 4) & 0x0f;
  const newHeader = (addressType << 4) | (networkId & 0x0f);
  const newBuf = Buffer.concat([Buffer.from([newHeader]), buf.slice(1)]);
  ...
  const words = bech32.toWords(newBuf);
  return bech32.encode(hrp, words, 200);
}
