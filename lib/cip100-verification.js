...
/**
 * CIP-100 Governance Metadata Verification Library
 * Based on cardano-signer's verify-cip100 implementation
 */

/**
 * Simple JSON canonicalization for governance metadata
 * This is a simplified version that handles the basic case
 * @param {Object} obj - Object to canonicalize
 * @returns {string}
 */
function simpleJSONCanonicalize(obj) {
  if (obj === null) return "null";
  if (typeof obj === "boolean" || typeof obj === "number") return String(obj);
  if (typeof obj === "string") return JSON.stringify(obj);

  if (Array.isArray(obj)) {
    return "[" + obj.map(simpleJSONCanonicalize).join(",") + "]";
  }

  if (typeof obj === "object") {
    const keys = Object.keys(obj).sort();
    const pairs = keys.map(
      (key) => JSON.stringify(key) + ":" + simpleJSONCanonicalize(obj[key])
    );
    return "{" + pairs.join(",") + "}";
  }
...
export async function verifyCIP100Metadata(metadata) {
  ...
  try {
    ...
    if (!parsedMetadata.authors || !Array.isArray(parsedMetadata.authors)) {
      throw new Error("Missing or invalid authors array in metadata");
    }
    ...
    if (result.result) {
      ...
      result.errorMsg = "One or more author signatures failed verification";
    }
  } catch (error) {
    result.result = false;
