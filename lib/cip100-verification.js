...
import { deriveEnterpriseAddress } from "./address.js";
...
function verifyAuthorSignature(author, canonizedHash) {
  ...
  try {
    ...
    // Derive signer address from public key on successful verification
    if (result.valid) {
      try {
        let rawKeyHex = author.witness.publicKey;
        // Strip COSE_Key prefix if present
        if (rawKeyHex.startsWith("a4010103272006215820")) {
          rawKeyHex = rawKeyHex.substring(20);
        }
        if (rawKeyHex.length === 64) {
          result.signerAddress = deriveEnterpriseAddress(rawKeyHex);
        }
      } catch {
        // Address derivation failure should not affect verification
      }
    }
  } catch (error) {
